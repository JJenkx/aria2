...
#  endif // !__ANDROID__ && !ANDROID
  ...
  {
    OptionHandler* op(new UnitNumberOptionHandler(
        PREF_MIN_SPLIT_SIZE, TEXT_MIN_SPLIT_SIZE, "20M", 16_k, 2047_m, 'k'));
    op->addTag(TAG_BASIC);
    op->addTag(TAG_FTP);
    op->addTag(TAG_HTTP);
    op->setInitialOption(true);
  ...
  {
    OptionHandler* op(new BooleanOptionHandler(
        PREF_ADAPTIVE_MIN_SPLIT_SIZE, TEXT_ADAPTIVE_MIN_SPLIT_SIZE, A2_V_FALSE,
        OptionHandler::OPT_ARG));
    op->addTag(TAG_BASIC);
    op->addTag(TAG_FTP);
    op->addTag(TAG_HTTP);
  ...
  {
    OptionHandler* op(new UnitNumberOptionHandler(
        PREF_PIECE_LENGTH, TEXT_PIECE_LENGTH, "1M", 16_k, 2047_m));
    op->addTag(TAG_ADVANCED);
    op->addTag(TAG_FTP);
    op->addTag(TAG_HTTP);
    op->setInitialOption(true);
